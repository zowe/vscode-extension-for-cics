...
export function getFilterProgramDefinitionsCommand(tree: CICSTree) {
  return commands.registerCommand(
    "cics-extension-for-zowe.filterProgramDefinitions",
    async (node) => {
      if (node) {
        const persistentFilters = new PersistentFilters("Zowe.CICS.Persistent");
        let pattern: string;
        const desc = new FilterDescriptor("\uFF0B Create New Program Definition Filter");
        const items = persistentFilters.getProgramSearchHistory().map(loadedFilter => {
