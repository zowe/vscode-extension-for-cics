...
export function getFilterDb2TransactionDefinitionCommand(tree: CICSTree, treeview: TreeView<any>) {
  return commands.registerCommand(
    "cics-extension-for-zowe.filterDb2TransactionDefinitions",
    async (node) => {
      const selection = treeview.selection;
      let chosenNode: CICSDb2TransactionDefinitionTree;
      if (node) {
        chosenNode = node;
      } else if (selection[selection.length-1] && selection[selection.length-1] instanceof CICSDb2TransactionDefinitionTree) {
        chosenNode = selection[selection.length-1];
      } else { 
        window.showErrorMessage("No CICS db2 transaction definition tree selected");
        return;
      }
      const persistentStorage = new PersistentStorage("zowe.cics.persistent");
      //do it for DB2
      ...
      window.withProgress({
        ...
        token.onCancellationRequested(() => {
          console.log("Cancelling the loading of db2 transaction definitions");
        });
        await chosenNode.loadContents();
        tree._onDidChangeTreeData.fire(undefined);
      });
