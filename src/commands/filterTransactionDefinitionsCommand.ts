...
export function getFilterTransactionDefinitionsCommand(tree: CICSTree) {
  return commands.registerCommand(
    "cics-extension-for-zowe.filterTransactionDefinitions",
    async (node) => {
      if (node) {
        const persistentFilters = new PersistentFilters("Zowe.CICS.Persistent");
        let pattern: string;
        const desc = new FilterDescriptor("\uFF0B Create New Transaction Definition Filter");
