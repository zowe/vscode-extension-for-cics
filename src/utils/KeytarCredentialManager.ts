...
config({
  ...
  bundleFormat: BundleFormat.standalone,
})();
...
export class KeytarCredentialManager extends AbstractCredentialManager {
  ...
  protected async deleteCredentials(account: string): Promise<void> {
    if (!(await this.deleteCredentialsHelper(account))) {
      Logger.getAppLogger().debug(
        localize(
          "errorHandling.deleteCredentials",
  ...
  protected async loadCredentials(
    ...
    const loadHelper = async (service: string) => {
      let secureValue: string = await KeytarCredentialManager.keytar.getPassword(
        service,
        account
      );
  ...
  protected async saveCredentials(
    ...
    await KeytarCredentialManager.keytar.setPassword(
  ...
  private async deleteCredentialsHelper(
    account: string,
    skipPreferredService?: boolean
  ): Promise<boolean> {
    ...
    for (const service of this.allServices) {
      ...
      if (
        await KeytarCredentialManager.keytar.deletePassword(service, account)
      ) {
        wasDeleted = true;
      }
    }
  ...
  private getMissingEntryMessage(account: string) {
    return (
      ...
      localize(
        "credentials.missingEntryMessage.2",
        "  Service = {0}\n",
        this.allServices.join(", ")
      ) +
      localize(
        "credentials.missingEntryMessage.3",
        "  Account = {0}\n\n",
        account
      ) +
